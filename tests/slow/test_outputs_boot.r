...
test_that("bootstrap CIs run for all functions that support them",{
    ...
    pmatrix.msm(psor.msm, ci="boot", B=2)
    ...
    pmatrix.msm(pci.msm, ci="boot", B=2)
})
...
test_that("normal CIs run for all functions that support them",{
    ...
    ppass.msm(psor.msm, tot=10, ci="normal", B=10) 
})
...
test_that("bootstrap CIs with factor covariates",{
    ...
    pmatrix.msm(psor2.msm, ci="boot", B=2, covariates=list(hieffusn=0, ollwsdrt="foo"))
})
